...
var passportSetup = function passportSetup(options,callback) {
  ...
  /*
  if (rc === undefined) {
    return callback(new Error('Error creating Passport Redis Client'));
  }
  */

  // other events ready, connect , error , end , drain , idle
  rc.on('error', function (err) {
    logger.log('error','Error with redis for passport %s', err.message);
    self.emit('error',err);
    return callback(err);
  });
  ...
  strategyTypes.forEach(function(strategyType) {
    ...
    var Strategy = require('passport-'+strategyType).Strategy;
    // Create a new strategy
    var strategy = new Strategy( strategyOptions[strategyType], strategyUserFindFunctions[strategyType]);
...
util.inherits(passportSetup, events.EventEmitter);
